...
import { AlertTriangle, CheckCircle2, KeyRound, Loader2, RefreshCw, ShieldCheck, UserPlus, Users } from 'lucide-react'
...
function AdminUsersScreen({ currentUser = null, onCurrentUserChanged = null }) {
  ...
  const loadUsers = async () => {
    ...
    setError('')
    try {
      const response = await apiV1Client.listAdminUsers()
      setUsers(Array.isArray(response) ? response : [])
    } catch (err) {
      setError(authFailureMessage(err, '사용자 목록을 불러오지 못했습니다.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const runAction = async ({ key, successMessage, username = '', action }) => {
    ...
    try {
      ...
      const currentUserChanged = await refreshIfCurrentUserChanged(username)
