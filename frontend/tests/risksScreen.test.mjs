...
assert.match(source, /Risk loading/)
assert.match(source, /Risk unavailable/)
assert.match(source, /summary\.yellow > 0 \|\| summary\.unknown > 0/, 'Yellow and unknown risks must remain a Check state')
assert.match(source, /No risks in response/, 'An empty legacy response must remain neutral rather than green')
assert.match(source, /total: '-'/, 'Unavailable Risks must not render as a healthy zero')
assert.match(source, /위험 데이터가 unavailable 상태입니다/)
