...
assert.equal(authFailureMessage({ status: 401, code: 'LOGIN_FAILED' }, 'fallback'), '계정명 또는 비밀번호가 올바르지 않습니다.')
...
const accountMenuSource = readFileSync(new URL('../src/app/AccountMenu.jsx', import.meta.url), 'utf8')
...
assert.match(appShellSource, /<AccountMenu currentUser=\{currentUser\}/, 'App shell must provide the authenticated user to the account menu')
assert.match(accountMenuSource, /currentUser\?\.username/, 'Account menu must show current username')
assert.match(accountMenuSource, /currentUser\?\.role/, 'Account menu must show current role')
...
assert.match(navigationSource, /label: '내 계정'/, 'Account settings nav item must be available for authenticated users')
