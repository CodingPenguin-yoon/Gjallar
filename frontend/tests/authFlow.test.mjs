...
assert.match(appSource, /apiV1Client\.changePassword\(form\.currentPassword, form\.newPassword\)/, 'App must change passwords through the auth API')
...
assert.match(appSource, /<InstanceList currentUser=\{currentUser\} canStartVms=\{canMutate\} \/>/, 'VM Inventory must not receive DRS policy management permission')
assert.match(appSource, /<DrsPoliciesScreen currentUser=\{currentUser\} canManageDrsPolicies=\{canMutate\}/, 'DRS Policies screen must receive role permission')
...
assert.match(appSource, /const settingsSubnavItems = isAdmin \? \[accountNavItem, adminNavItem\] : \[accountNavItem\]/, 'Settings subnavigation must keep Admin Users admin-only')
assert.match(appSource, /Account/, 'Account settings nav item must be available for authenticated users')
...
assert.match(drsPoliciesSource, /function DrsPoliciesScreen\(\{ currentUser = null, canManageDrsPolicies = false \}\)/, 'DRS Policies screen must accept session user and mutation permission props')
assert.match(drsPoliciesSource, /DRS policy review is visible, but updates require operator or admin role/, 'DRS Policies screen must keep policy reads visible to viewers')
assert.match(drsPoliciesSource, /policyReviewDisabledReason\(item, canManageDrsPolicies/, 'DRS policy update controls must be gated by role')
assert.match(drsPoliciesSource, /submitDrsPolicyUpdate/, 'DRS policy updates must go through the shared API helper')
assert.match(drsPoliciesSource, /policyChangeAcknowledged: acknowledged/, 'DRS policy writes must include explicit acknowledgement through the helper')
assert.match(drsPoliciesSource, /expectedObservation: item\.expectedObservationPayload/, 'DRS policy writes must include the expected observation guard')
assert.doesNotMatch(drsPoliciesSource, /approved_actor|operator_id|actor:|updated_by:|source:/, 'DRS policy payload must not include a frontend-supplied actor/source/operator')
...
assert.doesNotMatch(drsSource, /DRS policy updates require operator or admin role/, 'DRS Advisor must not expose DRS policy update role messaging')
assert.doesNotMatch(drsSource, /submitDrsPolicyUpdate/, 'DRS Advisor must not expose DRS policy update helper calls')
assert.doesNotMatch(drsSource, /PolicyReviewModal/, 'DRS Advisor must not render the DRS policy review modal')
