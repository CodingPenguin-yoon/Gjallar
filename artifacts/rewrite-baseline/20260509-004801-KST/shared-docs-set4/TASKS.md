...
Status: planned
Goal: Proxmox nodes/VMs/templates/storage/network/guest-agent/IP read-only 조회 adapter를 fake/read-only fixtures로 먼저 계약화하고 최소 GREEN으로 만든다.
Next action: next worker re-checks Set 4 focused GREEN tests, then writes focused RED tests for `app.proxmox` read-only inventory adapter/API shape before production code.
Done criteria summary: Set 5 focused RED→GREEN tests pass with unittest fallback; adapter has no create/apply/delete/power methods; secrets/log payloads are redacted; `git diff --check` pass; docs updated; no commit/push/delete/live side effect
Blocker: backend `.venv` still lacks pytest; Set 5 may use verified `unittest` fallback, but full backend pytest GREEN requires non-destructive dependency recovery before claim
...
Ordered task list:
- [ ] 1. Re-check repo status and Set 4 focused tests are GREEN
- [ ] 2. Write focused RED tests for read-only nodes/VMs/templates/storage/network/guest-agent/IP adapter contract before production code
- [ ] 3. Write/confirm forbidden tests that the adapter exposes no create/apply/delete/power methods
- [ ] 4. Implement minimal fake/read-only `app.proxmox` inventory models/adapter
- [ ] 5. Wire `/api/v1` inventory skeleton to the fake/read-only adapter only if covered by focused tests
- [ ] 6. Ensure secret/log redaction for adapter payloads
- [ ] 7. Run focused `unittest`, `git diff --check`, record repo status, update CURRENT_STATE.md/TASKS.md/logs

Done criteria:
- [ ] Set 5 focused RED tests were observed before production code
- [ ] focused Set 5 tests pass with `PYTHONPATH=backend backend/.venv/bin/python -m unittest ...`
- [ ] read-only adapter returns nodes/VMs/templates/storage/network/guest-agent/IP fixture shape
- [ ] adapter has no create/apply/delete/power methods or live mutation path
- [ ] secret/log payloads are redacted
- [ ] No production code was written outside Set 5 scope
...
ssh codex-vm 'cd /home/yoon/projects/Gjallar && git status --short --branch && git diff --check'
ssh codex-vm 'cd /home/yoon/projects/Gjallar && PYTHONPATH=backend backend/.venv/bin/python -m unittest backend/tests/jobs/test_artifact_store.py backend/tests/jobs/test_job_models.py'
# Set 5 worker must add and run its own focused Set 5 unittest command after RED is written.
```
