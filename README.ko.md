...
갈랴르는 템플릿 준비, VM 관리, 인프라 모니터링과 문제 복구를 더 쉽게 연결하는 도구를 만들고 있습니다. Proxmox 관리 화면을 오가지 않고, 갈랴르의 웹이나 CLI에서 일상 운영을 끝내는 것이 목표입니다.

실제 서버를 움직이는 엔진과 상태의 기준은 Proxmox입니다. 갈랴르는 현재 상태를 이해하고, 필요한 변경을 수행하고, 결과를 확인하는 과정을 연결합니다.

> **현재 개발 중입니다.** 지금은 웹에서 자원 조회, 기존 템플릿 기반 VM 생성, 시작·정상 종료와 작업 이력 확인을 제공합니다. CLI, 부트스트랩 설치, 템플릿 제작, 내장 VM 콘솔과 백업·복원 흐름은 아직 구현되지 않은 목표입니다.
...
| CLI·웹 | 어느 인터페이스에서도 같은 권한·작업·결과 사용 |
...
- 현재 모니터링 범위는 자원 관찰과 운영 인사이트입니다. 완전한 시계열 지표·알림 시스템은 아직 없으며, VM 실행 상태만으로 내부 애플리케이션의 정상 동작을 보장하지 않습니다.

템플릿 제작, 일반 VM 수정·삭제, 내장 콘솔, 백업·복원, 마이그레이션과 인프라 설정 변경은 미구현입니다. 현재 VM 생성에는 기존 Proxmox 템플릿이 필요하며, ISO 설치와 빈 VM 생성은 지원하지 않습니다.

## 현재 애플리케이션 실행하기

아직 단일 명령 설치 도구는 없습니다. 의존성 설치, 환경 설정, PostgreSQL 초기화와 계정 생성은 [운영 Runbook](project-docs/development.md)을 따릅니다.
