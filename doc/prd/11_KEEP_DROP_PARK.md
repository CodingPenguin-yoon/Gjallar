...
Last updated: 2026-05-09 10:49 KST
...
|---|---|---|---|
| Direct `/deploy` and `/provision` execution flow | `backend/app/domains/deploy/router.py`, `service.py`, legacy frontend API client | Contains reusable ideas but contract is wrong: it bypasses draft/plan/review/approval. | 2026-05-09: backend deploy package and legacy Terraform integration backed up and moved to archive; frontend client cleanup remains later scope. |
...
Backend blocker to resolve before claiming GREEN: `pytest` is installed in the current Codex VM backend `.venv`; full backend pytest now runs but remains RED only for missing `/api/v1/jobs` and `/api/v1/risks` read APIs after the legacy archive cleanup.
...
- Backend test dependency was recovered in the current Codex VM `.venv`; remaining full pytest blockers are `/api/v1/jobs` and `/api/v1/risks`.
