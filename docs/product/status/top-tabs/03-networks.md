...
검증 기준: worker가 backend `PYTHONPATH=backend python3 -m pytest -q backend/tests` -> 112 passed, frontend `node --test frontend/tests/*.mjs` -> 11 passed, `pnpm --dir frontend build` -> passed, `git diff --check` -> passed를 기록했다.
