...
`../24_DRS_ADVISOR_MVP_PRD.md`는 숫자형 PRD navigation을 위한 index로만 사용한다.
...
Create VM Terraform/GitOps flow는 삭제하지 않는다.
현재 구현된 보조 capability로 유지하고, DRS Advisor는 기존 Dashboard, Placement, Jobs/Runs, Risks/Alerts, Proxmox inventory, job/artifact substrate를 확장한다.
...
- Backend `/api/v1`은 read-only Proxmox inventory, Jobs/Runs, Risks, Create VM draft/preflight/plan/approval/GitOps/Terraform gates를 제공한다.
...
- Create VM flow는 `general-vm` 중심 draft/preflight/plan, artifact-backed review, approval checksum, GitOps manifest commit/archive, Terraform plan/apply acknowledgement gate를 갖고 있다.
