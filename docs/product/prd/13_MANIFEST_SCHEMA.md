...
Current implementation gap: 현재 code는 아직 built-in/current profile 경로,
`general-vm` only enabled, `network_id`/`server-net`, static `prefix`/`gateway`
미요구 상태다. 아래 target schema는 구현 업데이트 전 current code로 읽지 않는다.
