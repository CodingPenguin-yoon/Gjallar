...
- 현재 code는 세 개의 enabled/read-only `static_seed` profile을 사용한다. DB seed source는 target/future다.
...
현재 구현은 `general-vm`, `runtime-server`, `development-vm` 세 profile을
enabled/read-only `static_seed` choice로 제공한다. DB-seeded source는 아직
target/future다.
