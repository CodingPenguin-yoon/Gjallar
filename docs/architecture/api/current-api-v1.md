...
| Jobs/artifacts | Create VM, VM start, DRS, and bootstrap readiness intent steps write job status and artifacts to `job_runs` and `job_artifacts` through `GJALLAR_DATABASE_URL`. |
...
| Bootstrap readiness intent | `POST /nodes/{node_id}/vms/{vmid}/bootstrap-readiness-intents` records an operator-only, no-live-SSH/no-Ansible readiness intent from read-only inventory evidence. It is separate from Create VM success. |
...
| `POST /api/v1/drs/recommendations/{recommendation_id}/approval-packets` | Create local DRS approval packet and pending migration job intent. | Current check result, trusted operator session actor, and checksummed artifacts. | Writes local approval/job/artifact rows only. | Backend/API only in Goals 1-6; no broad DRS execution UI. | Operator-only. Does not call Proxmox and does not start migration. |
| `POST /api/v1/drs/migration-jobs/{job_id}/execute` | Execute one approved DRS migration job through the narrow live migration path. | Stored approval/job binding, fresh final pre-check, live Proxmox DRS evidence, operation locks, dedicated DRS migration client. | Acquires operation locks, calls Proxmox QEMU migrate through the DRS client, stores UPID/task/post-check evidence, releases locks only after verified success or marks `needs_reconciliation`. | Backend/API only in Goals 1-6; broad UI remains Goal 7. | Operator-only. No recommendation-level migrate/live-migrate aliases. No live DRS smoke has been run. |
| `POST /api/v1/drs/migration-jobs/{job_id}/reconcile-preview` | Read-only reconciliation evidence preview for a DRS migration job. | Stored job/UPID plus read-only task and post-check evidence. | None. | Backend/API only in Goals 1-6. | Operator-only, `proxmox_mutation_enabled=false`, no corrective mutation. |
...
| `POST /api/v1/nodes/{node_id}/vms/{vmid}/bootstrap-readiness-intents` | Record a post-create bootstrap readiness intent without live SSH/Ansible/app execution. | Fresh read-only inventory adapter evidence bound by `node_id` + `vmid`. | Writes local `bootstrap_readiness` job and `bootstrap_readiness_intent` DB artifact only; no Proxmox mutation, SSH socket/login, Ansible, guest exec, VM start, or app bootstrap. | Backend/API only. | Operator-only. Requires `bootstrap_readiness_acknowledged=true` and non-empty `idempotency_key`; rejects credential/execution payload keys without storing raw values; blocks templates, missing/moved targets, expected name/status/IP mismatches, non-running VMs, and missing guest-agent IP evidence. Duplicate idempotency returns the existing result. |
...
The frontend client does not expose Terraform plan/apply, legacy GitOps execute/archive helpers, Networks write helpers, bootstrap readiness intent, or DRS execution helpers, and the legacy backend routes no longer exist. The active UI covers inventory, the explicit VM start action, read-only network readiness, DRS Advisor read/check recommendations and manual VM policy configuration, jobs, risks, Create VM readiness, draft/preflight/plan/approve, native preview, and native create. Bootstrap readiness intent is backend/API-only, and broad DRS execution UI remains outside the current UI surface.
