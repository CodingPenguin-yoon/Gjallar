...
The frontend client does not expose Terraform plan/apply, legacy GitOps execute/archive helpers, Networks write helpers, or DRS execution helpers, and the legacy backend routes no longer exist. The active UI covers inventory, the explicit VM start action, read-only network readiness, DRS Advisor read/check recommendations, jobs, risks, Create VM readiness, draft/preflight/plan/approve, native preview, and native create. Broad DRS execution UI remains Goal 7.
