...
| `POST /api/v1/vm-create/{draft_id}/proxmox-create` | Active live native Proxmox VM creation. | Approved rebuilt plan and Proxmox mutation client. | Revalidates approval and may refresh `approval.json`; writes an internal preview artifact; calls Proxmox clone/task/config/resize/post-check; writes job/artifacts. | Create VM final mutation button. | Requires valid approval, no red fresh risk, and `proxmox_mutation_acknowledged=true`. It does not require `manifest_commit_sha`. Native config uses reviewed username and transient SSH public key; API/artifacts return only redacted/safe evidence. |
