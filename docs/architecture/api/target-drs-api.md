...
4. Operator-only `/migration-jobs/{job_id}/execute` first requires exact `drs_live_migration_acknowledged=true`; missing or malformed acknowledgement is request validation only and has no job-state side effects.
5. Backend validates approved job bindings/checksums.
6. Backend reruns a fresh final pre-check against current Proxmox and Gjallar identity/policy/lock state.
7. Backend collects live Proxmox DRS evidence for active tasks, HA, quorum, and migration preconditions.
8. Backend takes operation locks for VM identity, Proxmox locator, and route.
9. Backend starts Proxmox migration through the dedicated DRS migration client and records UPID.
10. Backend polls the Proxmox task to terminal or ambiguous state.
11. Backend post-checks VM target location, power state, fingerprint, and active task evidence.
12. Backend records `completed` only after verified post-check; otherwise it records `needs_reconciliation` and marks locks `reconciliation_required`.
13. Read-only reconcile preview can reread evidence. No corrective mutation or background automation exists.
