...
Gjallar는 human-facing Proxmox Operations Console입니다. 현재 앱은 operational visibility, gated stopped-VM start action, guided VM creation with stopped/boot-and-verify policies, read-only network readiness/migration pre-check, read-only DRS Advisor Phase 1 recommendations, DB-backed job history, job-derived risk summaries를 제공합니다.
...
| Gjallar-owned data | Create VM request/VM records, intent manifests, approval evidence, job records, artifacts, future identity/reconciliation data |
