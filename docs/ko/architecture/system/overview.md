...
Gjallar는 human-facing Proxmox Operations Console입니다. 현재 앱은 operational visibility, gated stopped-VM start action, guided VM creation with stopped/boot-and-verify policies, read-only network readiness/migration pre-check, read-only placement recommendations, DB-backed job history, job-derived risk summaries를 제공합니다.
