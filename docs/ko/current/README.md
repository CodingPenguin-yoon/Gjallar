...
Gjallar의 현재 baseline은 read-only Proxmox inventory, Dashboard aggregation, Infra Explorer, Networks bridge/policy view, read-only Placement, Jobs/Runs, Risks/Alerts, 그리고 Create VM supporting capability입니다.
...
Create VM은 강한 보조 capability입니다. 현재 active 생성 경로는 Proxmox native create이며, `execute`는 manifest commit only입니다.
