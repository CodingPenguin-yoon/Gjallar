...
현재 [frontend/src/utils/placement.js](../../../../frontend/src/utils/placement.js)는 frontend-only read model입니다. Node pressure는 CPU/Memory current usage의 max이고, source pressure와 imbalance delta로 candidate를 만듭니다. Red-risk VM은 제외하고 bridge/storage evidence를 보지만 execution은 `available: false`입니다.

이 로직은 target recommendation seed로 참고할 수 있지만 execution authority가 아닙니다.
