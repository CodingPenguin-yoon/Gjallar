...
Gjallar는 현재 Proxmox 운영자가 클러스터 상태, DRS Advisor Phase 1 후보, network readiness/migration pre-check, job/artifact, risk를 확인하고, 승인 기반으로 VM을 생성할 수 있게 하는 Proxmox Operations & Risk Console입니다. 기본 생성은 powered-off이고, 선택하면 `boot_and_verify`로 first boot/IP/cloud-init까지 확인합니다. DRS Advisor는 read-only Phase 1이고 backend DRS 실행 시스템은 없습니다.
...
| `/drs` DRS Advisor | backend-owned read-only Phase 1 recommendation seed | [Placement / DRS Advisor](current/top-tabs/05-placement-drs-advisor.md) |
...
- Current `/drs`는 backend DRS read-only endpoint를 사용합니다. 모든 recommendation은 `executable=false`이고 migration route는 없습니다.
