...
`/drs/policies`는 current non-template VM의 policy coverage를 `vm_identity_id` 기준으로 표시하며 현재 frontend에서는 `/instances/drs-policies`가 소비한다. `PUT /drs/policies/{vm_identity_id}`는 operator-only Gjallar-local manual update이며 `expected_observation` guard, `policy_change_acknowledged=true`, trusted session actor, and audit event row를 요구한다. `allowed`는 DRS prerequisite only이고 migration approval이 아니다. `unknown`, `restricted`, `blocked`는 계속 DRS-blocking이다.
...
- Frontend: [frontend/src/components/DrsAdvisorScreen.jsx](../../../frontend/src/components/DrsAdvisorScreen.jsx), [frontend/src/components/DrsPoliciesScreen.jsx](../../../frontend/src/components/DrsPoliciesScreen.jsx), [frontend/src/utils/drsAdvisor.js](../../../frontend/src/utils/drsAdvisor.js), [frontend/src/services/apiV1.js](../../../frontend/src/services/apiV1.js), [frontend/src/App.jsx](../../../frontend/src/App.jsx)
...
Manual policy configuration is attached only to Gjallar `vm_identity_id`. The shared policy review modal is used by `/instances/drs-policies`. It shows current locator and observation guard evidence, blocks policy writes for uncertain identities, requires a deliberate policy select, reason, acknowledgement, and does not send actor/source/operator fields from the browser. `/drs` does not render the policy edit grid or own policy update state.
...
화면은 review model only notice, recommendation/detail/check controls, criteria taxonomy, compact identity/policy evidence, and local approval packet/job intent controls를 표시한다. Broad migration 실행 UI와 policy edit UI는 제공하지 않는다.
...
Live execute confirm modal, broad UI execution controls, corrective reconciliation workflow가 없다. 현재 reconciliation surface는 backend read-only preview와 Operations/Jobs/operation-lock evidence에 한정된다.
