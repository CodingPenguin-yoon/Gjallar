...
현재 VM Instances inventory는 canonical `/instances` route의 VM/node inventory와 VM detail evidence 화면이며, legacy `/infra` deep link도 같은 화면을 렌더링한다. Stopped non-template VM에 한해 gated Start action을 제공한다. Inventory table은 DRS policy coverage를 보조 조회하지 않고, DRS Policy column, policy warning/success message, policy review modal을 렌더링하지 않는다. Manual policy configuration은 dedicated `/instances/drs-policies` screen으로 분리되어 있다. DRS migration action flow는 없다.
...
DRS policy state는 Inventory row에 join하지 않는다. Policy read/update workflow는 VM Instances subtab인 `/instances/drs-policies`가 담당한다.

Start action은 Create VM과 분리되어 있다. UI는 VM name, node, VMID, current status를 확인시키고 acknowledgement checkbox를 요구한 뒤 `/operations/jobs?job=<job_id>`로 이동한다. Legacy `/jobs?job=<job_id>`도 deep link로 유지된다. Backend는 fresh inventory에서 `(node_id, vmid)` exact match를 확인하고 missing/moved/template/non-stopped VM을 차단한다.
...
[DRS Advisor data/identity 목표](../../product/drs-advisor/03_DATA_DB_AND_IDENTITY.md) 대비 VM owner, profile, metadata completeness, sensitivity, identity assertion, fingerprint match/mismatch 상세 상태는 아직 VM Instances row에서 모두 표시하지 않는다. Migration policy 상태와 guarded review action은 `/instances/drs-policies`로 분리되어 있다.
...
VM Instances live inventory는 Proxmox current state 확인에는 적합하지만, metadata 자동 재연결 기준으로 쓰면 안 된다. DRS PRD는 fingerprint match 기반 재연결과 same VMID/different fingerprint 차단을 요구한다.
