...
Current implementation note: target payload의 DB seeded profile source is still
target/future, but current code already exposes three enabled read-only
`static_seed` profiles and uses live template/network inventory with explicit
`bridge_id`, `static_ip`, `prefix`, and `gateway`.
...
Current MVP product source of truth is `docs/product/drs-advisor/`.
