...
> This inventory may preserve old Create VM assumptions. Current implemented
> state is [`../../current/README.md`](../../current/README.md). Current Create
> VM profiles are three enabled read-only `static_seed` profiles:
...
This inventory is now read through the DRS Advisor direction. Create VM remains a supporting existing capability, but it must not define the next MVP success line or implementation order. The next implementation sequence is `docs/product/drs-advisor/05_IMPLEMENTATION_PLAN.md`.
...
[`../../architecture/CREATE_VM_PROFILE_TEMPLATE_NETWORK_DESIGN.md`](../../architecture/CREATE_VM_PROFILE_TEMPLATE_NETWORK_DESIGN.md).
Current code uses three enabled read-only `static_seed` profiles and live
template/network inventory. Create VM networking uses explicit active live
bridge selection and static `static_ip`/`prefix`/`gateway`; incoming
...
The existing code is **not** the product source of truth. `docs/product/drs-advisor/` is the current MVP product source of truth; existing modules are only material to Keep / Drop / Park after evidence-based review.

## 1. Inventory scope and evidence

- Shared PRD source of truth: `docs/product/drs-advisor/`
...
| Read-only Proxmox inventory and VM detail | Keep | `backend/app/proxmox/inventory.py`, `backend/app/proxmox/models.py`, `backend/tests/proxmox/test_inventory_adapter.py` | Current inventory adapter supports live read-only Proxmox data with fake fallback and redaction. It must stay mutation-free. |
...
| Create VM preflight/readiness | Keep as supporting capability | `backend/app/vm_create/*`, `backend/tests/vm_create/*` | Current active flow uses draft -> preflight -> plan -> Review & Confirm -> manifest commit -> Proxmox native preview/create. Current gap versus target: profiles are transitional `static_seed` instead of DB-seeded, while all three initial choices are enabled. Create VM networking uses explicit active live bridge selection and static `static_ip`/`prefix`/`gateway`. Reuse approval/artifact/UPID/observed_after ideas for DRS, but do not use Create VM as the next MVP success line. |
...
| Terraform integration code | Legacy optional | `backend/app/vm_create/terraform_runner.py`, `infra/terraform/main.tf`, Terraform safety tests | Terraform remains available as optional/deprecated executor code, but active UI Create VM does not call it. |
| Network/IP policy helpers | Keep for Network tab/current support | `backend/app/network_policy.py`, `backend/tests/contracts/test_api_v1_network_policy.py` | Current implementation uses IaC NetworkPolicy plus Proxmox observed bridge inventory. Target Create VM should select an active live bridge after target node selection and should not use NetworkPolicy/`network_id` as source of truth. |
...
| Create wizard UX shell | Keep | `frontend/src/components/CreateInstanceWizard.jsx`, `frontend/src/utils/createVmFlow.js`, `frontend/tests/createVmFlow.test.mjs` | Current wizard uses `/api/v1` draft/preflight/plan/approval/manifest commit/Proxmox native preview/create gates. Target gap: show DB-seeded `general-vm`/`runtime-server`/`development-vm`, live Proxmox template disabling by requirement, target node -> active bridge selection, static `prefix`/`gateway`, and no `network_id` dependency. Legacy provisioning helpers are removed from active `src`. |
...
| Group | Status for rewrite | Evidence | Notes |
|---|---|---|---|
| Terraform VM clone/provisioning base | Legacy optional | `infra/terraform/main.tf`, Terraform safety tests | The Terraform-backed VM create path is not the active enterprise default. Keep it as deprecated/optional until removal is explicitly approved. |
...
| Repo-local architecture/provisioning docs | Park | `docs/README.md`, `docs/architecture/*`, `docs/archive/features/*`, `docs/archive/operations/*`, `docs/archive/roadmap/*` | `docs/architecture/*` can describe current code architecture and DRS target gaps; `docs/archive/features`, `docs/archive/operations`, and `docs/archive/roadmap` are historical context. Current product source of truth is `docs/product/drs-advisor/`. |
...
Next DRS Advisor work starts from `docs/product/drs-advisor/05_IMPLEMENTATION_PLAN.md`.

Set 2 originally started with RED tests, not implementation. Priority RED targets were:

1. `/api/v1` response/error shape.
2. Target profile/template/network schema for DB-seeded `general-vm`, `runtime-server`, `development-vm`, Proxmox live templates, active live bridge selection, DHCP/static, required static `prefix`/`gateway`, hardware defaults/limits, SSH-key-required access, and password login disabled.
