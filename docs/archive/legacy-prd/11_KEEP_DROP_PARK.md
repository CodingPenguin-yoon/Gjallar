...
This create-first classification is historical/supporting capability context only. It must not define the next MVP success line or implementation order. The next implementation sequence is `docs/product/drs-advisor/05_IMPLEMENTATION_PLAN.md`.
...
| Repo-local docs | `docs/**`, `README.md` | `docs/architecture/*` can describe current code architecture; `docs/archive/features`, `docs/archive/operations`, and `docs/archive/roadmap` are historical. | Keep DRS Advisor source of truth in `docs/product/drs-advisor/`; add historical banners where older docs could be misread. |
