"""Infrastructure-free placement recommendation calculation.

This module owns inventory normalization, pressure scoring, candidate
selection, and route evidence for the canonical Insights placement read
boundary. It does not resolve DRS identities, read migration policy/locks, or
...
READ_ONLY_EXECUTION = {
    "available": False,
    "allowed_actions": [],
    "reason": "Placement output is observe-only and does not provide an execution path.",
}
...
ADVISORY_AUTHORITY = "advisor_prefilter_signal"
...
def _source_label(adapter: Any, snapshot: Any | None) -> str:
    return _as_text(
        _field(snapshot, "source", default=None) or _field(adapter, "source", default="unknown"),
        "unknown",
    )
...
def _storage_id(source: Any) -> str:
    return _as_text(
        _field(source, "storage_id", "storageId", "storage", "id", default="unknown"),
        "unknown",
    )
...
def _node_id(source: Any) -> str:
    return _as_text(
        _field(source, "node_id", "nodeId", "node", "id", "name", default="unknown"),
        "unknown",
    )
...
def _normalize_network(source: Any) -> dict[str, Any]:
    ...
    return {
        "bridge_id": _as_text(
            _field(source, "bridge_id", "bridgeId", "bridge", "id", default="unknown"),
            "unknown",
        ),
        "node_id": _node_id(source),
...
def _normalize_node(
    ...
    node_id = _node_id(source)
    inline_storages = [
        _normalize_storage(item)
        for item in _as_list(_field(source, "storage", default=[]))
    ]
    inline_networks = [
        _normalize_network(item)
        for item in _as_list(_field(source, "networks", default=[]))
    ]
    node_storages = inline_storages or [
        item for item in storages if item["node_id"] == node_id
    ]
    node_networks = inline_networks or [
        item for item in networks if item["node_id"] == node_id
    ]
    cpu_usage = _as_float(
        _field(source, "cpu_usage_percent", "cpuUsagePercent", default=0)
    )
    memory_usage = _as_float(
        _field(source, "memory_usage_percent", "memoryUsagePercent", default=0)
    )
    return {
        "id": node_id,
        "name": _as_text(
            _field(source, "display_name", "displayName", "name", default=node_id),
            node_id,
        ),
        "status": _normalize_status(_field(source, "status", default="unknown")),
        "online": _normalize_status(_field(source, "status", default="unknown"))
        == "online",
        "cpu_usage_percent": cpu_usage,
        "memory_usage_percent": memory_usage,
        "pressure": max(cpu_usage, memory_usage),
        "memory_total_mb": _as_int(
            _field(source, "memory_total_mb", "memoryTotalMb", default=0)
        ),
...
def _vm_bridge_ids(vm: Any) -> list[str]:
    evidence = _as_list(
        _field(vm, "nic_bridge_evidence", "nicBridgeEvidence", default=[])
    )
    return _unique(
        [
...
def _vm_mac_addresses(vm: Any) -> list[str]:
    direct = _as_list(
        _field(vm, "mac_addresses", "macAddresses", default=[])
    )
    nic_macs = [
    ...
    return _unique(
        [str(item).strip().lower() for item in [*direct, *nic_macs]]
    )
...
def _normalize_vm(source: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "vmid": _as_int(vmid),
        "name": _as_text(
            _field(source, "name", "vm_name", default=f"vm-{vmid}"),
            f"vm-{vmid}",
        ),
        "node_id": _as_text(
            _field(source, "node_id", "nodeId", "node", default="unknown"),
            "unknown",
        ),
        "status": _normalize_status(_field(source, "status", default="unknown")),
        ...
        "memory_mb": _as_int(
            _field(source, "memory_mb", "memoryMb", default=0)
        ),
        "disk_gb": _as_float(
            _field(source, "disk_gb", "diskGb", default=0)
        ),
        ...
        "config_lock": _as_text(
            _field(source, "config_lock", "configLock", default="")
        ),
...
def _risk_level(risk: Any) -> str:
    return _normalize_status(
        _field(risk, "level", "risk_level", "riskLevel", default="unknown")
    )
...
def _has_red_risk(vm: dict[str, Any], risks: list[Any]) -> bool:
    return any(
        _risk_level(risk) == "red" and _risk_targets_vm(risk, vm)
        for risk in risks
    )
...
def _passthrough_evidence(vm: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "method": "tags_only",
        "limitation": (
            "Current passthrough detection uses VM tags only; "
            "Proxmox device config parsing is not yet included."
        ),
    }
...
def _route_evidence(
    ...
    target_storage_ids = {
        storage["id"] for storage in target_node["storage"]
    }
    vm_storage_ids = set(vm["storage_ids"])
    matching_bridges = sorted(vm_bridge_ids & target_bridge_ids)
    source_bridge_present = not vm_bridge_ids or bool(
        vm_bridge_ids & source_bridge_ids
    )
    network_sufficient = (
        bool(vm_bridge_ids)
        and source_bridge_present
        and vm_bridge_ids.issubset(target_bridge_ids)
    )
    storage_sufficient = (
        bool(vm_storage_ids)
        and vm_storage_ids.issubset(target_storage_ids)
    )
    if storage_sufficient and vm["disk_gb"] > 0:
        matching_target_storages = [
            storage
            for storage in target_node["storage"]
            if storage["id"] in vm_storage_ids
        ]
        storage_sufficient = any(
            storage["free_gb"] >= vm["disk_gb"]
            for storage in matching_target_storages
        )
    return {
        ...
        "severity": (
            "warning"
            if not (network_sufficient and storage_sufficient)
            else "info"
        ),
        "evidence_state": (
            "observed"
            if network_sufficient and storage_sufficient
            else "unknown"
        ),
        "action_blocked": "none",
...
def _estimate_pressure_effect(
    vm: dict[str, Any],
    source_node: dict[str, Any],
) -> float:
    running_count = max(
        _as_int(source_node.get("running_vm_count"), 1),
        1,
    )
    ...
    if source_node["memory_total_mb"] > 0:
        memory_share = (
            vm["memory_mb"] / source_node["memory_total_mb"]
        ) * 100
...
def _candidate_reason(
    ...
    source_state = (
        "critical"
        if source_node["pressure"] >= THRESHOLDS["critical"]
        else "hot"
    )
    return (
...
def build_candidate_recommendation(
    *,
    ...
    """Build one neutral, execution-closed placement recommendation."""
    ...
    target_pressure_after = min(
        100,
        target_node["pressure"] + pressure_effect,
    )
    target_over_threshold = (
        target_pressure_after >= THRESHOLDS["critical"]
    )
    blockers = ["target_over_threshold"] if target_over_threshold else []
    delta = source_node["pressure"] - target_node["pressure"]
    return {
        "id": (
            f"drs-rec-vm-{_safe_segment(vm['vmid'])}-"
            f"{_safe_segment(source_node['id'])}-"
            f"{_safe_segment(target_node['id'])}"
        ),
        ...
        "criteria": {
            ...
            "advisory_signal_codes": [
                ...
                for code, evidence in (
                    ("route_unknown", route),
                    ("local_storage_dependency", local_storage),
                    ("passthrough_device_dependency", passthrough),
                )
                if evidence["blocked"]
            ],
            "technical_gate_status": "not_applicable",
            "authorities": [ADVISORY_AUTHORITY],
        },
        "criteria_details": [],
        "advisory_signals": [],
        "technical_gate_status": {
            "authority": "placement_observation",
        ...
        "estimated_effect": {
            ...
            "source_pressure_after": round(
                max(0, source_node["pressure"] - pressure_effect),
                2,
            ),
        ...
        "evidence": {
            ...
            "source": {
                ...
                "memory_usage_percent": source_node[
            ...
            "target": {
                ...
                "memory_usage_percent": target_node[
                    "memory_usage_percent"
                ],
            ...
            "target_over_threshold": {
                ...
                "projected_target_pressure_percent": round(
...
def _build_recommendations(
    ...
    sources = sorted(
        [
            node
            for node in nodes
            if node["online"] and node["pressure"] >= THRESHOLDS["hot"]
        ],
    ...
    for source_node in sources:
        target_candidates = [
            ...
            and source_node["pressure"] - node["pressure"]
            >= THRESHOLDS["source_target_delta"]
        ]
    ...
    return sorted(
        recommendations,
        key=lambda item: (
            -item["estimated_effect"]["source_target_delta"],
            item["vm_name"],
        ),
    )[:10]
...
def _cluster_state(
    ...
    if any(
        node["pressure"] >= THRESHOLDS["critical"] for node in nodes
    ):
        return "critical"
    if recommendations or any(
        node["pressure"] >= THRESHOLDS["hot"] for node in nodes
    ):
        return "hot"
    return "balanced"
...
def _summary(
    ...
    running_non_template = [
        vm
        for vm in vms
        if vm["status"] == "running" and not vm["template"]
    ]
    excluded_red = [
        vm for vm in running_non_template if _has_red_risk(vm, risks)
    ]
    online_nodes = [node for node in nodes if node["online"]]
    pressures = [node["pressure"] for node in online_nodes]
    pressure_delta = (
        max(pressures) - min(pressures) if len(pressures) >= 2 else 0
    )
    return {
        ...
        "hot_node_count": len(
            [
                ...
                if node["pressure"] >= THRESHOLDS["hot"]
            ]
        ),
        "critical_node_count": len(
            [
                node
                for node in online_nodes
                if node["pressure"] >= THRESHOLDS["critical"]
            ]
        ),
...
def calculate_placement(
    ...
    *,
    ...
    observed_at = _as_text(
        _field(snapshot, "observed_at", default="")
    )
    storages = [
        _normalize_storage(item)
        for item in _adapter_items(
            adapter,
            snapshot,
            "list_storage",
            "storage",
    ...
    model = {
        ...
        "evidence": {
            ...
            "candidate_filter": (
                "running_non_template_vms_without_red_risk"
            ),
...
def build_placement_model(
    ...
    *,
    ...
    """Build the canonical observe-only placement read model."""
    return calculate_placement(adapter, risks=risks)["model"]
