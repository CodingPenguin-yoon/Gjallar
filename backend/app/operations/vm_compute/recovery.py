"""GET-only compute recovery. Missing dispatch acknowledgements remain paused."""
...
class ComputeRecoveryHandler:
    ...
    def handle(self, lease):
        ...
        if operation is None:
            raise RuntimeError("Compute recovery Operation is missing")
        try:
            _validate_recovery_binding(lease, operation, recovery_kind="vm_compute_observation",
                operation_type="vm_compute", execution_mode="managed_api", target_lock_reader=get_target_operation_lock)
        ...
        if operation.status == "planned" and operation.details.get("mutation_dispatched") is False:
            self.recovery.commit_observation(
                lease, next_status="blocked", event_type="compute_interrupted_before_dispatch", stage="precheck",
                details_patch={"failure_code": "VM_COMPUTE_INTERRUPTED_BEFORE_DISPATCH"},
        ...
        try:
            ...
            self.recovery.commit_observation(lease, event_type="compute_recovery_unavailable", stage="reconciliation",
