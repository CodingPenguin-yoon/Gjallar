...
class DiskRecoveryHandler:
    ...
    def handle(self, lease):
        ...
        if operation is None:
            raise RuntimeError("Disk recovery Operation is missing")
        try:
            _validate_recovery_binding(lease, operation, recovery_kind="vm_disk_observation",
                operation_type="vm_disk_resize", execution_mode="managed_api", target_lock_reader=get_target_operation_lock)
        ...
        if operation.status == "planned" and operation.details.get("mutation_dispatched") is False:
            self.recovery.commit_observation(lease, next_status="blocked", event_type="disk_interrupted_before_dispatch",
                stage="precheck", details_patch={"failure_code": "VM_DISK_INTERRUPTED_BEFORE_DISPATCH"},
        ...
        try:
            ...
            self.recovery.commit_observation(lease, event_type="disk_recovery_unavailable", stage="reconciliation",
