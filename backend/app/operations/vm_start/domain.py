"""Infrastructure-free VM Start command and stable intent rules."""
...
def vm_start_target(*, node_id: str, vmid: int, name: str = "") -> dict[str, Any]:
    return {"node_id": str(node_id), "vmid": int(vmid), "name": str(name)}


def vm_start_target_lock_id(vmid: int) -> str:
    return f"vmid:{int(vmid)}"


def vm_start_expected_context(payload: Mapping[str, Any]) -> dict[str, Any]:
    return {
...
def stable_vm_start_intent(*, node_id: str, vmid: int, payload: Mapping[str, Any]) -> dict[str, Any]:
    return {
...
class VmStartCommand:
    """Application input independent of FastAPI and concrete adapters."""
    ...
    def expected(self) -> dict[str, Any]:
        return vm_start_expected_context(self.payload)

    @property
    def stable_intent(self) -> dict[str, Any]:
        return stable_vm_start_intent(node_id=self.node_id, vmid=self.vmid, payload=self.payload)
