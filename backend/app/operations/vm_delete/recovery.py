...
class DeleteRecoveryHandler:
    ...
    def handle(self, lease):
        ...
        if operation is None:
            raise RuntimeError("Delete recovery Operation is missing")
        try:
            _validate_recovery_binding(lease, operation, recovery_kind="vm_delete_observation",
                operation_type="vm_delete", execution_mode="managed_api", target_lock_reader=get_target_operation_lock)
        ...
        if operation.status == "planned" and operation.details.get("mutation_dispatched") is False:
            self.recovery.commit_observation(lease, next_status="blocked", event_type="delete_interrupted_before_dispatch",
                stage="precheck", details_patch={"failure_code": "VM_DELETE_INTERRUPTED_BEFORE_DISPATCH"},
        ...
        try:
            ...
            self.recovery.commit_observation(lease, event_type="delete_recovery_unavailable", stage="reconciliation",
