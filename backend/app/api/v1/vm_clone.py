"""Authenticated review and execution of stopped VM clone changes."""
...
from app.operations.vm_clone.domain import CloneError, CloneRequest
...
def _invoke(action, **kwargs):
    try:
        return getattr(clone_service(), action)(**kwargs)
    except CloneError as exc:
        raise HTTPException(exc.status_code, detail=exc.to_detail()) from None
    except OperationIntentConflict:
        raise HTTPException(409, detail={"code": "VM_CLONE_IDEMPOTENCY_CONFLICT", "message": "요청 ID가 다른 변경에 사용됐습니다."}) from None
    except DurableTargetLockBusy as exc:
        reason = exc.existing.get("reason", "VM_CLONE_TARGET_BUSY")
        ...
        raise HTTPException(503, detail={"code": "VM_CLONE_PERSISTENCE_UNAVAILABLE",
            "message": "작업 기록·복구 권한을 확인하지 못했습니다. 같은 요청 ID의 Operation을 확인하고 자동 재실행하지 마세요."}) from None


@router.get("/nodes/{node_id}/vms/{vmid}/clone")
async def review_clone(node_id: str, vmid: int, new_vmid: int, storage_id: str, actor: AuthenticatedUser = Depends(require_operator)):
    return success_response(await run_in_threadpool(_invoke, "review", node_id=node_id, vmid=vmid, new_vmid=new_vmid, storage_id=storage_id))


@router.post("/nodes/{node_id}/vms/{vmid}/actions/clone")
async def execute_clone(node_id: str, vmid: int, payload: CloneRequest,
