"""Authenticated review and execution of stopped VM delete changes."""
...
def _invoke(action, **kwargs):
    try:
        return getattr(delete_service(), action)(**kwargs)
        ...
        raise HTTPException(409, detail={"code": "VM_DELETE_IDEMPOTENCY_CONFLICT", "message": "요청 ID가 다른 변경에 사용됐습니다."}) from None
    except DurableTargetLockBusy as exc:
        reason = exc.existing.get("reason", "VM_DELETE_TARGET_BUSY")
        ...
        raise HTTPException(503, detail={"code": "VM_DELETE_PERSISTENCE_UNAVAILABLE",
            "message": "작업 기록·복구 권한을 확인하지 못했습니다. 같은 요청 ID의 Operation을 확인하고 자동 재실행하지 마세요."}) from None


@router.get("/nodes/{node_id}/vms/{vmid}/deletion")
async def review_delete(node_id: str, vmid: int, actor: AuthenticatedUser = Depends(require_operator)):
    return success_response(await run_in_threadpool(_invoke, "review", node_id=node_id, vmid=vmid))


@router.post("/nodes/{node_id}/vms/{vmid}/actions/delete")
async def execute_delete(node_id: str, vmid: int, payload: DeleteRequest,
