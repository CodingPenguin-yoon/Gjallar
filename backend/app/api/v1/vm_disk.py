"""Authenticated review and execution of stopped VM disk expansion."""
...
from app.operations.vm_disk.domain import DiskError, DiskRequest
...
def _invoke(action, **kwargs):
    try:
        return getattr(disk_service(), action)(**kwargs)
    except DiskError as exc:
        raise HTTPException(exc.status_code, detail=exc.to_detail()) from None
    except OperationIntentConflict:
        raise HTTPException(409, detail={"code": "VM_DISK_IDEMPOTENCY_CONFLICT", "message": "요청 ID가 다른 변경에 사용됐습니다."}) from None
    except DurableTargetLockBusy as exc:
        reason = exc.existing.get("reason", "VM_DISK_TARGET_BUSY")
        ...
        raise HTTPException(503, detail={"code": "VM_DISK_PERSISTENCE_UNAVAILABLE",
            "message": "작업 기록·복구 권한을 확인하지 못했습니다. 같은 요청 ID의 Operation을 확인하고 자동 재실행하지 마세요."}) from None


@router.get("/nodes/{node_id}/vms/{vmid}/disks/scsi0")
async def review_disk(node_id: str, vmid: int, actor: AuthenticatedUser = Depends(require_operator)):
    return success_response(await run_in_threadpool(_invoke, "review", node_id=node_id, vmid=vmid))


@router.post("/nodes/{node_id}/vms/{vmid}/actions/disk-resize")
async def execute_disk(node_id: str, vmid: int, payload: DiskRequest,
