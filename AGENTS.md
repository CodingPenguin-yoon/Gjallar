...
하위 문서나 현재 구현은 상위 제품 방향을 재해석하거나 확장할 수 없다. 코드와 테스트는 현재 상태의 근거이지 장기 목표의 근거가 아니다.

작업 전에는 현재 상태, 제거·호환성 유지를 포함한 과도기 상태, 최종 목표 상태를 구분한다. “지금 제거할 수 없음”을 “장기 유지 또는 신규 통합 승인”으로 해석하지 않는다. 제거·대체 대상으로 승인된 기능은 안전, 보안, 데이터 보존을 위한 수정 외에 신규 기능, 자동화, 통합 또는 의존성을 추가하지 않는다. 예외는 그 확장 자체를 사용자가 명시적으로 승인한 경우뿐이다.

중요한 결정을 요청할 때는 선택지 뒤에 “이 승인은 X를 의미하며 Y를 의미하지 않는다”는 승인 문장을 한 문장으로 재진술한다. 사용자의 짧은 동의는 바로 앞의 승인 문장에 직접 답한 경우에만 그 범위의 승인으로 기록한다. 승인 범위가 불분명하거나 기존 방향과 충돌하면 구현하지 않고 차이를 설명해 다시 확인한다.

긴 작업을 재개하거나 대화 맥락이 요약·압축된 경우에는 최신 제품 방향, Specification 상태, ADR의 `ACCEPTED`·`SUPERSEDED` 관계, Plan 상태를 다시 확인한다. 과거 Plan이나 거절·대체된 ADR을 현재 권한으로 사용하지 않는다.
...
- 요구사항과 완료 조건이 충족되었다.
- 변경이 승인된 최종 목표를 앞당기며 제거·대체 대상을 불필요하게 확장하지 않았다.
