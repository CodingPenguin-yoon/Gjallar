...
3. 최신 사용자 제품 방향과 문서 상태를 대조하고 대체·거절·롤백된 결정을 현재 권한으로 사용하지 않는다.
4. 호출자, 공개 계약, 상태, 데이터 접근, transaction, 외부 효과를 조사한다.
5. 기존 테스트와 characterization test 가능성을 확인한다.
6. 변경 빈도, 결함, 테스트 비용 같은 실제 문제를 근거로 정리한다.

현재 아키텍처, 과도기 호환성 상태, 목표 아키텍처를 섞어 설명하지 않는다. 제거·대체 대상이 현재 존재한다는 이유만으로 새 구조에 통합하거나 확장하지 않는다.
...
권장 방향을 새 ADR의 `PROPOSED` 상태와 고위험 공유 Plan의 `DRAFT` 상태로 작성한다. 주요 결정을 한 번에 하나씩 묻고 “이 승인은 X를 의미하며 Y를 의미하지 않는다”는 문장으로 허용 범위와 비범위를 재진술한다. 사용자가 그 문장에 동의한 뒤 ADR을 `ACCEPTED`, Plan을 `APPROVED`로 전환한다. 승인 전에는 되돌리기 어려운 구현을 시작하지 않는다.
